/**
 * 408-ADDR-1G — Places API (New) address autocomplete.
 *
...
(() => {
  ...
  const parsePlace = (place) => {
    ...
    return {
      formattedAddress: place?.formattedAddress || place?.formatted_address || input.value.trim(),
      ...
      country: componentValue(components, 'country', 'short'),
  ...
  suggestionList.id = `${input.id || 'property-address'}-suggestions`;
  suggestionList.setAttribute('role', 'listbox');
  suggestionList.setAttribute('aria-label', 'Likely property addresses');
  ...
  const resetSession = () => {
    sessionToken = AutocompleteSessionToken ? new AutocompleteSessionToken() : null;
  };
  ...
  const selectPrediction = async (prediction) => {
    ...
    helper.textContent = 'Confirming the selected address…';
    ...
    try {
      ...
      if (!address.formattedAddress) throw new Error('Selected place did not return an address.');
      ...
      console.warn('Selected address could not be confirmed. Manual entry remains available.', error);
      clearStructuredAddress();
      helper.textContent = 'We could not confirm that suggestion. You can keep entering the address manually.';
    } finally {
  ...
  const renderSuggestions = (predictions) => {
    ...
    if (!currentPredictions.length) {
      closeSuggestions();
      helper.textContent = 'No likely match yet. Keep typing or enter the address manually.';
      return;
    }
    ...
    helper.textContent = 'Choose the best matching address, or keep typing to enter it manually.';
  };

  const requestSuggestions = async (query) => {
    if (!placesReady || !AutocompleteSuggestion) return;

    const requestId = ++newestRequestId;
    input.dataset.addressAutocompleteBusy = 'true';

    try {
      if (!sessionToken) resetSession();
      ...
      if (requestId !== newestRequestId || input.value.trim() !== query) return;
      ...
      console.warn('Address suggestions are unavailable. Manual entry remains available.', error);
      closeSuggestions();
      setState('unavailable');
    } finally {
      if (requestId === newestRequestId) input.dataset.addressAutocompleteBusy = 'false';
    }
  };
  ...
  const scheduleSuggestions = () => {
    ...
    if (!placesReady || query.length < MIN_QUERY_LENGTH) {
      newestRequestId += 1;
      closeSuggestions();
      return;
    }
  ...
  input.addEventListener('keydown', (event) => {
    const listOpen = !suggestionList.hidden && currentPredictions.length > 0;
    ...
    if (event.key === 'ArrowDown' && listOpen) {
      ...
      event.preventDefault();
      setActiveSuggestion(activeIndex <= 0 ? currentPredictions.length - 1 : activeIndex - 1);
    } else if (event.key === 'Enter' && listOpen && activeIndex >= 0) {
      event.preventDefault();
      void selectPrediction(currentPredictions[activeIndex]);
  ...
  document.addEventListener('pointerdown', (event) => {
    if (!fieldLabel?.contains?.(event.target)) closeSuggestions();
  });
  ...
  if (existingScript) {
    existingScript.addEventListener('load', () => { void initializeAutocomplete(); }, { once: true });
  ...
  window[GOOGLE_READY_CALLBACK] = settleLoaded;
  ...
  const query = [
    ...
    'loading=async',
    `callback=${encodeURIComponent(GOOGLE_READY_CALLBACK)}`,
    'v=weekly'
  ].join('&');
  ...
  loadTimeout = window.setTimeout(settleUnavailable, SCRIPT_LOAD_TIMEOUT_MS);
